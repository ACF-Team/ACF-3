...
function ENT:GetConsumption(Throttle, RPM, FuelTank, SelfTbl)
	...
	if SelfTbl.FuelType == "Electric" then
		return Throttle * SelfTbl.FuelUse * SelfTbl.Torque * RPM * 1.05e-4 * (1 + 4 * (1 - self.FuelCrewMod))
	else
		local IdleConsumption = SelfTbl.PeakPower * 5e2
		return SelfTbl.FuelUse * (IdleConsumption + Throttle * SelfTbl.Torque * RPM) / FuelTank.FuelDensity * (1 + 4 * (1 - self.FuelCrewMod))
	end
end
