...
local function ActWheel(Link, Wheel, Torque, Brake, DeltaTime, Mass)
	...
	local IBR = 1-BrakeR--inverse ratio to "force clutch" on forward torque
	local TorqueAxis = Phys:LocalToWorldVector(Link.Axis)
	local ForwardTorque =  Clamp(math.deg( -Torque * 1.5 * IBR) * DeltaTime, -500000, 500000) 
	
	
	local WheelInertia = Phys:GetInertia()
	local WheelAngVel = Phys:LocalToWorldVector(Phys:GetAngleVelocity())
	local Stabilizer =  (-WheelInertia*WheelAngVel):Dot(TorqueAxis)
	local BrakeTorque = Clamp(BrakeR  * Stabilizer * (Mass/1000) ,-500000,500000)--Mass Multiplier to help heavy vehicles brake
	local TorqueMult = ForwardTorque+BrakeTorque
	--print(ForwardTorque, "+", BrakeTorque, "=", TorqueMult)

	Phys:ApplyTorqueCenter(TorqueAxis * TorqueMult)
end
...
function ENT:Act(Torque, DeltaTime, MassRatio)
	...
	for Ent, Link in pairs(self.Wheels) do
		...
		local BrakeMult = Link.Side == 0 and self.LBrake or self.RBrake

		ActWheel(Link, Ent, WheelTorque, BrakeMult, DeltaTime, Mass)
