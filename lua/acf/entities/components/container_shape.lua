...
Classes.DefineClass("ACF.ContainerShapes.BaseContainerShape", function() end)

for ShapeName, ShapeModel in pairs(ShapeModels) do
	local ClassName = "ACF.ContainerShapes." .. tostring(ShapeName)

	Classes.DefineClass(ClassName, "ACF.ContainerShapes.BaseContainerShape", function()
		CLASS.Name = ShapeName
		CLASS.Model = ShapeModel
		CLASS.ShapeCalculation = ShapeCalculations[ShapeName]
	end)
end

-- TODO: Is this actually necessary? Can this just be merged into Cylinder fully?
-- Alias for backwards compatibility
-- Classes.DefineClass("ACF.ContainerShapes.Drum", "ACF.ContainerShapes.Cylinder", function() end)